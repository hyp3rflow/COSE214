...
int changePW(int state){
	...
	if(w) return newState - WOLF;
	else return newState + WOLF;
}
...
int changePG(int state){
	...
	if(g) return newState - GOAT;
	else return newState + GOAT;
}
...
int changePC(int state){
	...
	if(CABBAGE) return newState - CABBAGE;
	else return newState + CABBAGE;
}
...
void dfs_main(int state, int goal_state, int level, int *visited){
	if(state == goal_state || visited[state]){
		visited[state] = level;
		return;
	}
	else{
		...
		for(int i=0; i<4; i++){
			newState = fp[i](state);
			if(is_possible_transition(state, newState) && !is_dead_end(newState) && !is_visited(visited, level, newState)){
