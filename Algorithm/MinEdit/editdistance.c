...
int min_editdistance(char *str1, char *str2){
	...
	int COST[3] = {INSERT_COST, DELETE_COST, SUBSTITUTE_COST};
	int OP[3] = {INSERT_OP, DELETE_OP, SUBSTITUTE_OP};
	int dx[3] = {0, -1, -1};
	int dy[3] = {-1, 0, -1};
	...
	for(int j=1; j<=m; j++){
		for(int i=1; i<=n; i++){
			...
			if(i > 1 && j > 1 && str1[i-1] == str2[j-2] && str1[i-2] == str2[j-1]){
				...
				else if(tmp == min_edit[row_size * i + j]) op_matrix[row_size * i + j] += TRANSPOSE_OP;
			}
		}
	}
